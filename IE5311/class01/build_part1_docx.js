...
push(
  ...
  body("First, does every feasible point correspond to a genuine s-to-t path? A feasible integer solution decomposes into one s-to-t path plus, possibly, some disjoint directed cycles. Since arc costs here are nonnegative, dropping any such cycle can only reduce cost, so an optimal solution carries no cycle and is a path. With negative cycles present the formulation would not be exact, which is a condition worth stating explicitly."),
...
push(
  ...
  gap(),
  body("The position constraints number on the order of n² rather than 2ⁿ, so the whole model fits in memory. The cost appears elsewhere. Slide 30 states the comparison precisely: the two describe the same feasible solution space, but one is tighter than the other. Relaxing integrality in the Dantzig, Fulkerson and Johnson model yields a bound much closer to the true optimum than the same relaxation of the Miller, Tucker and Zemlin model. Tighter bounds prune more of the branch-and-bound tree, which is why the exponential formulation with generated constraints outperforms the compact one in practice."),
...
push(
  ...
  lead("Numerical confirmation.", "For the six-node, nine-arc instance of Section 2, the incidence matrix has 6 rows and 9 columns. Enumerating every square submatrix up to order four gives 4,164 determinants, and all of them lie in {0, +1, −1}."),
